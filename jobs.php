...
<div class="grid"><?php foreach($jobs as $job): $spots=$job['applicant_limit']!==null?(int)$job['applicant_limit']:null; $tags=job_tags($job['tags']??''); ?>
<a data-row class="card job-card" href="job-detail.php?slug=<?=urlencode($job['slug'])?>">
<div class="job-card-top"><span class="pill"><?=e($job['department']??'Acme')?></span><?php if($job['is_urgent']): ?><span class="badge low">Urgent</span><?php endif; ?><?php if($spots!==null && $spots>0): ?><span class="badge low"><?=icon('candidates',11)?> <?=$spots?> <?=$spots===1?'spot':'spots'?></span><?php endif; ?></div>
